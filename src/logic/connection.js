...
function createRoom() {
  ...
  socket.emit("room create", {
    source: state.content.episode ? `eiswald://${state.content.episode.language}:${state.content.episode.season}@${state.content.episode.playlist}/${state.content.episode.key}/${state.content.episode.name}` : "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4",
    title: state.content.episode ? state.content.episode.name : "Unbekannt",
...
function sync() {
  const state = store.getState();
  socket.emit("sync", {
    source: state.content.episode ? `eiswald://${state.content.episode.language}:${state.content.episode.season}@${state.content.episode.playlist}/${state.content.episode.key}/${state.content.episode.name}` : state.player.source,
    title: state.content.episode ? state.content.episode.name : "Unbekannt",
    playing: state.player.playing,
    time: state.player.time
  });
...
socket.on("sync", (data) => {
  ...
  //store.dispatch(setTitle(data.content.title)); THERE IS NO TITLE
  store.dispatch(play(data.content.playing));
  store.dispatch(setTime(data.content.time));
